...
class PlateReader:
    ...
    def identify(self, image_org):
        ...
        id_res = cv2.resize(plate_ID, dsize=(300*2, 480), interpolation=cv2.INTER_CUBIC)[:, x0:x1]
        ...
        #         plate = cv2.resize(plate, dsize=(85, 280), interpolation=cv2.INTER_CUBIC)
        #         h, w = plate.shape
...
def main():
    ...
    id_model            = load_model('/home/andrew/ros_ws/src/2020T1_competition/controller/models/plate_id_model.h5')
    ...
    start = time.time()
    plate = plate_reader.identify(image)
    stop = time.time()
