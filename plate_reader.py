...
class PlateReader:
    ...
    def find(self, image_org):
        ...
        # Check contours and enclosed area to determine which is more likely the plate
        if screenCnt_lit is None and screenCnt_dark is None:
            #print("No contour detected")
            detected_dark = 0
            detected_lit = 0  
        elif screenCnt_lit is not None and screenCnt_dark is None:
            detected_lit = 1
        elif screenCnt_dark is not None and screenCnt_lit is None:
            detected_dark = 1
        else:
            area_lit = cv2.contourArea(screenCnt_lit)
            area_dark = cv2.contourArea(screenCnt_dark)
            if area_lit > area_dark:
                detected_lit = 1
            else:
                detected_dark = 1
        ...
        if warped.size > 20000 or warped.size < 1000:
            return "NO_PLATE", [], []
    ...
    def guess(self, plate_set, plate_id): 
        ...
        with self.graph.as_default():
            ...
            h, w = plate_id.shape
            plate_id = plate_id.reshape(1, h, w, 1)/255
    
            #s = time.time()
            predict = self.id_model.predict(plate_id)
            #print(time.time() - s)
...
def main():
    ...
    if (plate != "NO_PLATE"):
        p, pprob, i, iprob = plate_reader.guess(plate_chars, plate_id)
    stop = time.time()
